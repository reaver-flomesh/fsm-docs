...
### 2. 确认 FSM 的 init 容器已成功运行

FSM 的初始化容器 `fsm-init` 负责使用流量重定向规则初始化服务网格中的单个应用程序 pod，以通过 Pipy 代理 sidecar 来代理应用程序流量。流量重定向规则是使用一组 `iptables` 命令设置的，这些命令在 pod 中的任何应用程序容器运行之前运行。

通过在应用程序 pod 上运行 `kubectl describe` 来确认 FSM 的初始化容器已成功完成运行，并验证 `fsm-init` 容器已终止，退出代码为 0。容器的 `State` 属性提供了此信息。
...
### 1. 确认在 `fsm-mesh-config` MeshConfig 资源中正确配置了出站 IP 范围
...
当配置出站 IP 范围排除时，FSM 的 `fsm-injector` 服务从 `fsm-mesh-config` `MeshConfig` 资源中读取此配置，并编写与这些范围相对应的 `iptables` 规则，以便将它们排除在通过 Pipy sidecar 代理出站流量重定向之外。

确认 FSM 的 `fsm-init` 初始化容器规范具有与要排除的已配置出站 IP 范围相对应的规则。
...
### 1. 确认在 `fsm-mesh-config` MeshConfig 资源中正确配置了全局出站端口
...
当配置了出站端口排除时，FSM 的 `fsm-injector` 服务从 `fsm-mesh-config` `MeshConfig` 资源和 pod 上的注解中读取此配置，并编写与这些范围相对应的 `iptables` 规则，以便它们被排除在通过 Pipy sidecar 代理进行的出站流量重定向之外。

确认 FSM 的 `fsm-init` 初始化容器规范具有与配置的出站端口对应的规则以排除。
