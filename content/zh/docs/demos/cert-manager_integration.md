...
本指南演示了如何使用 [cert-manager][1] 作为证书提供程序在 FSM 中管理和颁发证书。

## 先决条件

- Kubernetes 集群版本 {{< param min_k8s_version >}} 或者更高。
- 使用 `kubectl` 与 API server 交互。
- 已安装 `fsm` 或者 `Helm 3` 命令行工具，用于安装 FSM 和 Contour。


## 演示

下面的演示使用 [cert-manager][1] 作为证书提供者，向 FSM 托管服务网格中的通过 `Mutual TLS (mTLS)` 通信的 `curl` 和 `httpbin` 应用颁发证书。
...
1. 为 `cert-manager` 配置颁发证书所需 `cert-manager` `Issuer` 和 `Certificate` 资源。 这些资源部署在随后安装 FSM 的同一个命名空间中。
    > 注意：必须先安装 `cert-manager` 并保证 issuer 就绪，随后将 `cert-manager` 作为证书提供者安装 FSM 。
...
1. 确保 `cert-manager` 在 FSM 命名空间下创建了 `fsm-ca-bundle` secret。
2. 
    ```console
    ...
    FSM 在安装时将使用保存在这个 secret 中的 CA 证书来引导其证书提供者程序。

3. 安装 FSM 时，指定其证书提供者为 `cert-manager`。
    ```bash
    fsm install --set fsm.certificateProvider.kind="cert-manager"
    ```

    确保 FSM 控制平面 pod 就绪并运行。
    ```console
