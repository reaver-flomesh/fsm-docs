...
description: 「FSM コントロール プレーン エラー コード トラブルシューティング ガイド」
aliases: ["/docs/troubleshooting/observability/errorcodes/"]
type: docs
---

# FSM コントロール プレーン エラー コード トラブルシューティング ガイド

## エラーコードの説明

エラー コードが FSM エラー ログに存在するか、FSM エラー コード メトリクスから検出された場合、「fsm support error-info」cli ツールを使用して、エラー コードに関する詳細情報を取得できます。

次の表は、「fsm support error-info」を実行して生成されます。
...
+------------+----------------------------------------------------------------------------------+
| E5010      | The Aggregate Discovery Server (ADS) created by the FSM controller failed to     |
|            | start.                                                                           |
+------------+----------------------------------------------------------------------------------+
...
特定のエラー コードの情報は、「fsm support error-info <error-code>」を実行することで取得できます。 例えば: 
```console
